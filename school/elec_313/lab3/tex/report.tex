...
As seen in Figure 2, as the peak voltage of the rectifier circuit increases (Figure 1 (a)), the rectifier dc voltage increases. Also, as load resistance is increased on the rectifier, it decreases the 

the increase of resistance reduced the amount of current that could be dissipated from the capacitor over the same amount of time 

before it was “charged up” again. Additionally, the
there would be a smaller time interval for the capacitor to discharge. 
As shown in Table 5, the VL across the 100 ohms resistor is 5.02 load resistor was removed. This shows that the Zener used in the experiment is not an ideal Zener.
In the regulator circuit, when VS is below the Zener diode voltage (VZ), VOC is linearly related to VS. When VS is above VZ, VOC 

is almost at a constant value approximately equal to VZ. When VS is “close” to VZ, the relationship to of VOC is not linearly 

related and the simplified calculations we learn in class are less effective at to computing the VOC. 
