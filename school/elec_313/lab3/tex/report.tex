...
%%% Logically, any of the tables above that refer to a percent difference should be moved to the comparison section, but...  my desire to actually do so is roughly nil.
\section{Comparison of Results}
\label{sec:comp_of_res}

%%% The comments about what the experimenter may've done belong more in the conclusion, if anywhere.  Also, not really sure what you're trying to get across.  The more likely reason(s) values were so close: testing equipment wasn't precise enough, and/or/mostly we didn't do an actual plot around the knee of the curve/it's a log function so values change rapidly around that point anyways.  The only thing we could've done better was to have more independent data-takers.
...
%%% There's no reason to refer to equations after the first time in such a short document.  Once in the procedure should be good enough.
